            ...
            try {
                ...
                if (storedData) {
                    ...
                    if (expiryTimestamp && expiryTimestamp > Date.now()) {
                        homepage(<Examresults query={data} />);
                        return;
                    } else {
                ...
                // temp
                // const url = "/api/multi?htno=" + htno + "&code=" + code;
                ...
                // temp
                const response = await axios.get(url + '/api/multi?htno=' + htno + "&code=" + code, { mode: 'cors' });
                ...
                if (response.status === 500) {
                    ...
                    homepage(<Examresults query={response.data} />);
                }
            } catch {
    ...
    return (
        <>
            ...
            <div method="get">
                <center>
                    <br />
                    <h1 className="mb-2 font-bold text-[180%]">Exam Results</h1>
                    <p>Get the semester results here</p>
                    <br />
                    <div>
                        <input
                            onChange={inputEvent}
...
const HomePage = () => {
    ...
    const [content, setContent] = useState(<ExamResults homepage={homepage} />);
