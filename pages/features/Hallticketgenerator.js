...
import Hallticketgenerate from "../../components/Features/Hallticketgenerates";


const HallticketGenerate = ({ homepage }) => {
      ...
      try {
        ...
        if (storedData) {
          ...
          if (expiryTimestamp > Date.now()) {
            homepage(<Hallticketgenerate query={data} />);
            return;
          } else {
        ...
        try {
          ...
          if (redisResponse.data && redisResponse.data !== "Internal server error" && redisResponse.data !== "Data not found in cache") {
            ...
            homepage(<Hallticketgenerate query={redisResponse.data} />);
            return;
          }
        } catch (error) {
        ...
        if (response.status === 500) {
          ...
          console.log('Cache Expiry', new Date(expiryTimestamp));
          homepage(<Hallticketgenerate query={response.data} />);
        }
      } catch {
  ...
  return (
    <>
      <Head>
        <title>
          JNTUH | HALLTICKET GENERATOR
        </title>
        <meta
          name="description"
          content="Check out hallticket for plan a studys ."
          key="desc"
        />
      </Head>
      <div method="get">
        <center>
          ...
          <p className="capitalize">easily view the entire supplementary exam schedule</p>
          <br />
...
const HomePage = () => {
  ...
  const [content, setContent] = useState(<HallticketGenerate homepage={homepage} />);
