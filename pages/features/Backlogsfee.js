...
import FeesEstimator from "../../components/Features/FeesEstimator";
      ...
      try {
        ...
        if (storedData) {
          ...
          if (expiryTimestamp > Date.now()) {
            homepage(<FeesEstimator query={data} />);
            return;
          } else {
        ...
        try {
          ...
          if (redisResponse.data && redisResponse.data !== "Internal server error" && redisResponse.data !== "Data not found in cache") {
            ...
            homepage(<FeesEstimator query={redisResponse.data} />);
            return;
          }
        } catch (error) {
        ...
        if (response.status === 500) {
          ...
          console.log('Cache Expiry', new Date(expiryTimestamp));
          homepage(<FeesEstimator query={response.data} />);
        }
      } catch {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Check out academic backlogs fee with in a go."
          key="desc"
        />
      </Head>
      <div method="get">
        <center>
          ...
          <p>Get Your Complete Backlogs Fee Estimation Here </p>
          <br />
...
const HomePage = () => {
  ...
  const [content, setContent] = useState(<Backlogsfee homepage={homepage} />);
