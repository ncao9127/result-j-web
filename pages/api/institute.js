...
export default async function handler(req, res) {
  ...
  const rollNumber = req.query['code'];
  ...
  try {
    ...
    if (cache) {
      console.log("Data found in cache for institute details:", rollNumber);
      const data = JSON.parse(cache);
      return res.status(200).json(data);
    } else {
      console.log("Data not found in cache for institute details:", rollNumber);
