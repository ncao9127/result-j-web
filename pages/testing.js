...
import Branch from '../components//Json/Branch_codes.json';
import College from '../components//Json/college_codes.json';

const ClassResultResult = ({ semester = '4-2' }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/classresult?semester=${semester}&htnos=${htnos.join(',')}`);
