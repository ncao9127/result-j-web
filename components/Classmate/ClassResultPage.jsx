    ...
    const submit = async () => {
        ...
        try {
            ...
            for (let reg = 0; reg < regularorLateral.length; reg++) {
                ...
                const url = "http://resultsjntuh.up.railway.app/api/classresult?semester=" + form['semesterOption'] + '&htnos=';
                for (let i = 0; i < roll_last_2.length; i++) {
                    ...
                    if (response.status === 200) {
                        ...
                        expiryDate.setHours(expiryDate.getHours() + 1.5); // Set expiry date to 1 hour from now
                        const dataToStore = {
                            value: response.data,
